...
void gf2_xor_scalar(uint64_t *dst, const uint64_t *src, size_t nwords) {
    for (size_t i = 0; i < nwords; i++) dst[i] ^= src[i];
}

uint64_t gf2_inner_scalar(const uint64_t *a, const uint64_t *b, size_t nwords) {
    uint64_t acc = 0;
...
uint64_t gf2_weight_scalar(const uint64_t *a, size_t nwords) {
    uint64_t acc = 0;
    for (size_t i = 0; i < nwords; i++)
